...
const DeleteService = ({ handleClose, deleteLoading, setDeleteLoading, deleteData }) => {

    const deleteItem = () => {
        setDeleteLoading(true)
        const docRef = doc(db, "services", deleteData?.id)
        deleteDoc(docRef)
        .then(() => {
  ...
  return (
    <div className='bg-[#fff] w-[400px] h-[368px] mt-[100px] rounded-lg flex flex-col gap-[32px]'>
        ...
        <div className='flex flex-col items-center justify-center px-5 gap-3'>
            ...
            <p className='text-[#000929] font-semibold text-[20px]'>Delete {deleteData?.title}</p>
            <p className='font-sans text-base text-center text-[#475569]'>
                Do you want to delete this service? This action can’t be undone
            </p>
